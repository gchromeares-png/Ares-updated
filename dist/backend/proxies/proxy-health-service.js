...
exports.ProxyHealthService = exports.parseProxycheckResponse = void 0;
...
function parseAttackHistory(value) {
...
function parseProxycheckResponse(exitIp, payload) {
...
exports.parseProxycheckResponse = parseProxycheckResponse;
function connectBase(proxy, timeoutMs) {
        ...
        const onData = (chunk) => {
            ...
            if (marker < 0) {
                if (data.length > 32 * 1024)
                    finish(new Error("CONNECT-Header zu groß."));
                return;
            }
...
function tlsToTarget(socket, targetHost, timeoutMs) {
    return new Promise((resolve, reject) => {
        let done = false;
        const secure = tls.connect({ socket: socket, servername: targetHost, rejectUnauthorized: true });
        const fail = (error) => {
            if (done)
                return;
            done = true;
...
async function httpTunnel(proxy, targetHost, targetPort, timeoutMs) {
...
async function socksTunnel(proxy, targetHost, targetPort, timeoutMs) {
    const socket = await connectBase(proxy, timeoutMs);
    ...
    if (greeting[0] !== 5 || greeting[1] === 0xff)
        throw new Error("SOCKS5 Auth-Methode abgelehnt.");
    if (greeting[1] === 2) {
        const user = Buffer.from(proxy.username ?? "", "utf8");
        const pass = Buffer.from(proxy.password ?? "", "utf8");
        if (user.length > 255 || pass.length > 255)
            throw new Error("SOCKS5 Login zu lang.");
        ...
        if (auth[1] !== 0)
            throw new Error("SOCKS5 Login abgelehnt.");
    }
    ...
    if (response[0] !== 5 || response[1] !== 0)
        throw new Error(`SOCKS5 Connect Code ${response[1]}.`);
    if (response[3] === 1)
        await readExact(socket, 6, timeoutMs);
    else if (response[3] === 4)
        await readExact(socket, 18, timeoutMs);
    else if (response[3] === 3) {
        const len = await readExact(socket, 1, timeoutMs);
        await readExact(socket, len[0] + 2, timeoutMs);
    }
    else
        throw new Error("SOCKS5 unbekannter Adresstyp.");
    return tlsToTarget(socket, targetHost, timeoutMs);
}
...
function bodyFromResponse(response) {
    ...
    if (marker < 0)
        throw new Error("Ungültige HTTP-Antwort vom IP-Check.");
    const headers = response.subarray(0, marker).toString("utf8");
    const status = headers.split("\r\n", 1)[0] ?? "";
    if (!/^HTTP\/\d(?:\.\d)? 2\d\d\b/.test(status))
        throw new Error(`IP-Check fehlgeschlagen: ${status}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        const onData = (chunk) => {
            ...
            if (size > MAX_RESPONSE_BYTES)
                return finish(new Error("IP-Check Antwort zu groß."));
            chunks.push(chunk);
        };
...
async function defaultProbeExitIp(proxy, timeoutMs) {
    ...
    const payload = JSON.parse(bodyFromResponse(response).toString("utf8"));
    const exitIp = String(payload.ip ?? "").trim();
    if (!exitIp)
        throw new Error("Exit-IP konnte nicht bestimmt werden.");
    return { exitIp, latencyMs: Date.now() - started };
}
function defaultLookupReputation(exitIp, timeoutMs) {
    return new Promise(resolve => {
        ...
        const request = https.get({
            ...
            response.on("end", () => {
                try {
                    if (size > MAX_RESPONSE_BYTES)
                        throw new Error("Reputation-Antwort zu groß.");
                    if ((response.statusCode ?? 500) >= 400)
                        throw new Error(`proxycheck.io HTTP ${response.statusCode}`);
...
class ProxyHealthService {
    ...
    async test(proxy, timeoutMs = DEFAULT_TIMEOUT_MS) {
...
exports.ProxyHealthService = ProxyHealthService;
//# sourceMappingURL=proxy-health-service.js.map