...
exports.ShopifyCheckoutJourney = void 0;
...
class ShopifyCheckoutJourney {
    ...
    async submitOrder(page, canPurchase) {
    ...
    async findMatchingControl(page, patterns, excludedPatterns = []) {
        ...
        for (let index = 0; index < count; index++) {
            ...
            if (!await this.isActionable(candidate))
                continue;
            const label = await this.controlLabel(candidate);
            if (!label || excludedPatterns.some(pattern => pattern.test(label)))
                continue;
...
exports.ShopifyCheckoutJourney = ShopifyCheckoutJourney;
//# sourceMappingURL=checkout-journey.js.map