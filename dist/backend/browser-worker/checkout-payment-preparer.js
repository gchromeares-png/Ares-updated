...
exports.CheckoutPaymentPreparer = void 0;
const METHODS = [
...
class CheckoutPaymentPreparer {
    async prepare(page, session) {
        ...
        if (selected || session.method === "card")
            result.selectedMethod = session.method;
    ...
    async hasVisibleCardField(page) {
    ...
    async selectMethod(page, method, preferredLabel) {
    ...
    async fillCardField(page, key, value, selectors, result) {
        if (!value?.trim()) {
            if (key !== "holderName")
        ...
        for (const frame of this.frames(page)) {
            for (const selector of selectors) {
                ...
                if (!await this.isVisible(locator))
                    continue;
                try {
                    await locator.fill(value.trim(), { timeout: 1500 });
...
exports.CheckoutPaymentPreparer = CheckoutPaymentPreparer;
//# sourceMappingURL=checkout-payment-preparer.js.map