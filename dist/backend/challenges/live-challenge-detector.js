...
exports.detectFromHtml = exports.detectLiveChallenge = exports.defaultDetector = exports.LiveChallengeDetector = void 0;
class LiveChallengeDetector {
    /**
     * Prüft einen HTML/URL-Snapshot auf Challenges (wird von den Snapshot-Tests verwendet).
     */
    detectFromSnapshot(url = "", html = "", title = "") {
        ...
        if (lowerHtml.includes("cf-turnstile") ||
        ...
        // 4. Google reCAPTCHA
        if (lowerHtml.includes("g-recaptcha") ||
        ...
        // 5. hCaptcha
        if (lowerHtml.includes("h-captcha") ||
        ...
        if (lowerUrl.includes("/checkpoint") ||
        ...
        if (typeof page.evaluate === "function") {
            try {
                const evalResult = await page.evaluate(() => {
                    const hasTurnstile = Boolean(document.querySelector('input[name="cf-turnstile-response"]') ||
                        document.querySelector('.cf-turnstile') ||
                        document.querySelector('iframe[src*="challenges.cloudflare.com"]') ||
                        document.querySelector('iframe[src*="turnstile"]'));
                    if (hasTurnstile)
                        return "turnstile";
                    const hasRecaptcha = Boolean(document.querySelector('textarea[name="g-recaptcha-response"]') ||
                        document.querySelector('.g-recaptcha') ||
                        document.querySelector('iframe[src*="google.com/recaptcha"]'));
                    if (hasRecaptcha)
                        return "recaptcha";
                    const hasHcaptcha = Boolean(document.querySelector('textarea[name="h-captcha-response"]') ||
                        document.querySelector('.h-captcha') ||
                        document.querySelector('iframe[src*="hcaptcha.com"]'));
                    if (hasHcaptcha)
                        return "hcaptcha";
                    const hasDataDome = Boolean(document.querySelector('iframe[title*="DataDome" i]') ||
                ...
                if (evalResult && typeof evalResult === "object") {
                    const detectedType = (evalResult.type || (evalResult.turnstile ? "turnstile" : undefined));
                    return {
...
exports.LiveChallengeDetector = LiveChallengeDetector;
exports.defaultDetector = new LiveChallengeDetector();
async function detectLiveChallenge(page) {
    return exports.defaultDetector.detect(page);
}
exports.detectLiveChallenge = detectLiveChallenge;
function detectFromHtml(html, url = "", title = "") {
    return exports.defaultDetector.detectFromSnapshot(url, html, title);
}
exports.detectFromHtml = detectFromHtml;
//# sourceMappingURL=live-challenge-detector.js.map