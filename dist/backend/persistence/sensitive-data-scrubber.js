...
exports.scrubLegacySensitiveData = exports.sanitizePersistedMessage = exports.sanitizePersistedValue = void 0;
...
function isDropSensitiveKey(key) {
 ...
 */
function scrubLegacySensitiveData(db) {
    const scrub = db.transaction(() => {
        const taskRows = db.prepare("SELECT id, config_json, last_error FROM tasks").all();
        ...
        const logRows = db.prepare("SELECT id, message FROM task_logs").all();
        ...
        for (const row of logRows) {
            ...
            if (sanitizedMessage !== rawMessage)
                updateLog.run(sanitizedMessage, id);
        }
        const eventRows = db.prepare("SELECT id, event_json FROM product_monitor_events").all();
        ...
        for (const row of eventRows) {
            ...
            try {
                ...
                if (sanitizedJson !== rawJson)
                    updateEvent.run(sanitizedJson, id);
            }
            catch {
...
exports.scrubLegacySensitiveData = scrubLegacySensitiveData;
//# sourceMappingURL=sensitive-data-scrubber.js.map