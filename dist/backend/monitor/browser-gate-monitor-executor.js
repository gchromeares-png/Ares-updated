...
exports.BrowserGateMonitorExecutor = exports.resolveMonitorNetworkMode = void 0;
const tslib_1 = require("tslib");
const os = tslib_1.__importStar(require("os"));
const path = tslib_1.__importStar(require("path"));
const queue_waiter_1 = require("../browser-worker/queue-waiter");
const session_http_poller_1 = require("./session-http-poller");
const early_gate_1 = require("./early-gate");
const monitor_policy_1 = require("./monitor-policy");
const POST_NAVIGATION_OBSERVATION_MS = 30000;
const RELEASE_STATUS_RE = /(released|complete|completed|redirect|passed|admitted)/i;
function proxyValue(profile) {
...
function sessionPollUrl(task, shop) {
...
function resolveMonitorNetworkMode(task, shop) {
    return (0, monitor_policy_1.resolveMonitorPolicy)(task, shop).networkMode;
}
exports.resolveMonitorNetworkMode = resolveMonitorNetworkMode;
...
function monitorLog(taskId, message) {
...
class BrowserGateMonitorExecutor {
    constructor(getShop, getProfile, browserWorker, options = {}) {
        this.getShop = getShop;
        this.getProfile = getProfile;
        this.browserWorker = browserWorker;
        this.active = new Map();
        this.runtimeListeners = new Set();
        this.pollIntervalMs = Math.min(10000, Math.max(250, options.pollIntervalMs ?? 750));
        this.refreshIntervalMs = Math.min(60000, Math.max(POST_NAVIGATION_OBSERVATION_MS, options.refreshIntervalMs ?? 5000));
    }
    ...
    async execute(task) {
        ...
        const policy = (0, monitor_policy_1.resolveMonitorPolicy)(task, shop);
        ...
        try {
            ...
            if (networkMode === "session-http-preferred") {
                sessionPoller = new session_http_poller_1.SessionHttpPoller({
                    url: pollUrl,
            ...
            const waiter = new queue_waiter_1.BrowserQueueWaiter(page, task, current => this.emit(current), {
                ...
                externalSignal: networkMode === "session-http-preferred" ? () => {
                    ...
                    if (!signal.active && !authoritativeRelease)
                        return undefined;
                    return {
                        active: signal.active,
            ...
            waiter.start();
            try {
                await page.goto(shop.baseUrl, { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => undefined);
                while (!controller.signal.aborted) {
                    publishSessionTelemetry();
                    const queue = await waiter.waitIfQueued();
                    if (queue.detected) {
                        const status = task.config.data?.["queueStatus"];
                        ...
                        (0, early_gate_1.setEarlyGateRuntime)(task, { activeArea: "gate", stage: "gate-detected", gateDetectedAt: detectedAt });
                        task.config.data = {
                            ...(task.config.data ?? {}),
                            browserGateHandoff: {
    ...
    emit(task) { for (const listener of this.runtimeListeners)
        listener(task); }
    delay(ms, signal) {
        if (signal.aborted)
...
exports.BrowserGateMonitorExecutor = BrowserGateMonitorExecutor;
//# sourceMappingURL=browser-gate-monitor-executor.js.map